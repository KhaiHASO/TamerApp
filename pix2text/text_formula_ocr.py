...
class TextFormulaOCR(object):
    ...
    def recognize(
        ...
        # Che phần công thức, sau đó OCR phần còn lại
        for mf_box_info in analyzer_outs:
        ...
        for line_box_info in text_box_infos['detected_texts']:
            # crop_img_info['box'] có thể là một hình chữ nhật có góc, cần chuyển đổi thành hình chữ nhật ngang
    ...
    def _post_process(self, outs):
        ...
        for idx, line_boxes in enumerate(outs):
            ...
            if len(line_boxes) > 1:
                ...
                # Gộp tag công thức vào trong công thức
                for _idx2, box in enumerate(line_boxes[1:]):
                    if (
                        box['type'] == 'text'
                        ...
                        if y_overlap(line_boxes[_idx2], box, key='position') > 0.9:
                            if re.match(formula_tag, box['text']):
                                # Loại bỏ dấu ngoặc ở đầu và cuối
    ...
    def _split_line_image(cls, line_box, embed_mfs):
        # Sử dụng vị trí của công thức embedding, cắt ảnh văn bản một dòng thành nhiều đoạn nhỏ, sau đó các đoạn này sẽ được đưa vào OCR để nhận diện văn bản
        line_box = line_box[0]
        if not embed_mfs:
...
# Dựa trên Vlm để triển khai một lớp con của TextFormulaOCR
