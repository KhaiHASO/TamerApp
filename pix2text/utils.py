...
def read_img(
    ...
    Args:
        ...
        return_type (str): Loại trả về;
            Hỗ trợ `Tensor`, trả về torch.Tensor; `ndarray`, trả về np.ndarray; `Image`, trả về `Image.Image`
    ...
    img = Image.open(path)
    img = ImageOps.exif_transpose(img)  # Nhận diện ảnh đã xoay (pillow không tự động nhận diện)
...
def get_background_color(image: Image.Image, margin=2):
    ...
    # Thống kê tần suất màu pixel ở viền
    color_counter = Counter(edge_pixels)

    # Lấy màu có tần suất cao nhất
...
def add_img_margin(
    ...
    # Tạo đối tượng ảnh mới và điền màu nền chỉ định
    new_image = Image.new("RGB", (new_width, new_height), background_color)

    # Dán ảnh gốc vào giữa ảnh mới
...
COLOR_LIST = [
    [0, 140, 255],  # Cam đậm
    [127, 255, 0],  # Xanh lá mùa xuân
    [255, 144, 30],  # Xanh dương Dodge
    [180, 105, 255],  # Hồng
    [128, 0, 128],  # Tím
    [0, 255, 255],  # Vàng
    [255, 191, 0],  # Xanh thiên đậm
    [50, 205, 50],  # Xanh lá vôi
    [60, 20, 220],  # Đỏ tươi
    [130, 0, 75],  # Chàm
    [255, 0, 0],  # Đỏ
    [0, 255, 0],  # Xanh lá
