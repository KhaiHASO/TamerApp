    ...
    def _cal_scores(self, outs):
        if isinstance(outs, GenerateBeamEncoderDecoderOutput):
            ...
            for idx, example in enumerate(scores):
                ...
                assert cur_length > 1
                # Lấy giá trị trung bình hình học. Lưu ý: phần tử đầu tiên trong example tương ứng với phần tử thứ hai trong sequence
                mean_probs.append(
...
def remove_empty_text(latex_expression):
    ...
    patterns = [
        ...
        r'_\s*{\s*}',  # Khớp _{}
        r'\\text\s*{\s*}',  # Khớp \text{}
        r'\\tilde\s*{\s*}',  # Khớp \tilde{}
        r'\\bar\s*{\s*}',  # Khớp \bar{}
        r'\\vec\s*{\s*}',  # Khớp \vec{}
        r'\\acute\s*{\s*}',  # Khớp \acute{}
        r'\\grave\s*{\s*}',  # Khớp \grave{}
        r'\\breve\s*{\s*}',  # Khớp \breve{}
        r'\\overline\s*{\s*}',  # Khớp \overline{}
        r'\\dot\s*{\s*}',  # Khớp \dot{}
        r'\\ddot\s*{\s*}',  # Khớp \ddot{}
        r'\\widehat\s*{\s*}',  # Khớp \widehat{}
        r'\\widetilde\s*{\s*}',  # Khớp \widetilde{}
    ]

    # Sử dụng hàm sub để thay thế
...
latex_whitespace_symbols = [
    r'\\ +',  # Khoảng trắng
    r'\\quad\s*',  # Khoảng trắng rộng 1em
    r'\\qquad\s*',  # Khoảng trắng rộng 2em
    r'\\,\s*',  # Khoảng trắng hẹp
    r'\\:\s*',  # Khoảng trắng trung bình
    r'\\;\s*',  # Khoảng trắng lớn
    r'\\enspace\s*',  # Khoảng trắng 1em
    r'\\thinspace\s*',  # Khoảng trắng 1/6em
    r'\\!\s*',  # Dấu chấm than
]


def remove_trailing_whitespace(latex_str):
    # Định nghĩa pattern regex để khớp các ký hiệu khoảng trắng ở cuối
